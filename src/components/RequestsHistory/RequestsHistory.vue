<template>
  <div class="requests_history_container">
    ...
    <div class="requests_history container container__material">
      ...
      <div v-for="(request, index) in requests" @click="$emit('showPopup', {command: requestsTranslate[request.command] || request.command, prompt: request.prompt, 'message': 'Хотите изменить результат?'})" class="requests_history_item requests_history_item__material">
