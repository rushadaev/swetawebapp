<template>
  <div class="requests_history_container">
    ...
    <div class="requests_history container container__material">
      ...
      <div v-for="(request, index) in requests.sort((a, b) => (a.id > b.id ? -1 : 1))" @click="$emit('showPopup', {command: request.command, prompt: request.related_requests.length > 0 ? request.related_requests[0].prompt : request.prompt, 'message': 'Хотите изменить результат?'})" class="requests_history_item requests_history_item__material">
